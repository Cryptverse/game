        ...
        for (let j = 0; j < this.amount; j++) {
            ...
            if (petal) {
                ...
                if (!petal.launched) {
                    ...
                    if (this.clumps) {
                        // Clump center
                        // const ang = i / nSpots * Math.PI * 2 + this.player.petalRotation; // REMOVED
                        const ang = (this.config.shooter && (this.player.attack || this.player.defend)) ? this.player.pointAngle : i / nSpots * Math.PI * 2 + this.player.petalRotation; // ADDITION
                        if (this.config.shooter) petal.facing = ang + 1.3; // ADDITION
                        ...
                        // const ang = (i + j) / nSpots * Math.PI * 2 + this.player.petalRotation; // REMOVED
                        const ang = (this.config.shooter && (this.player.attack || this.player.defend)) ? this.player.pointAngle : (i + j) / nSpots * Math.PI * 2 + this.player.petalRotation; // ADDITION
                    ...
                    // ADDITION
                    if (this.config.shooter && (this.player.attack || this.player.defend)) {
                        petal.x = gx;
                        petal.y = gy;
                        petal.facing = this.player.pointAngle;
                    }
                    // END
                    const dx = gx - petal.x;
                    const dy = gy - petal.y;
...
export class Petal extends Entity {
    ...
    define(config, rarity) {
        ...
        if (config.turret) {
            this.turret = {
                petal: new Petal().define(petalConfigs[config.turret.petal], rarity),
                offset: config.turret.offset
            }
            this.turret.petal.x = this.x; // change later
            this.turret.petal.y = this.y; // change later
        }
    }
    ...
    collide() {
        ...
        // Addition
        if (this.turret) {
            this.turret.petal.x = this.x + this.turret.offset.x; // change later
        }
            ...
            if (this.shotTick <= 0) {
                if (this.parent.attack){
                    ...
                    for (let i = 0; i < shooter.count; i++) {
                        ...
                        petal.pushability = 0;
                        petal.facing = this.facing + (Math.random() - 0.5) * shooter.spread;
                        ...
                        if (config.projectile?.friction == null) {
                            ...
                            petal.velocity.y = Math.sin(petal.facing);
                        }
                        else {
                            petal.friction = config.projectile.friction;
                            petal.velocity.x = Math.cos(petal.facing) * shooter.speed;
                            petal.velocity.y = Math.sin(petal.facing) * shooter.speed;
                        }
                        petal.launched = true;
                        petal.range = config.projectile?.lifetime || 50;
                        if (config.projectile?.seeking) {
                            petal.seekTarget = petal.findTargetAngleWithinRadianArc(petal.facing, 1.5)[1];
        ...
        if (this.seekTarget) {
            const targetAngle = Math.atan2(this.seekTarget.y - this.y, this.seekTarget.x - this.x);
            const currentAngle = this.velocity.angle;
            const speed = this.velocity.magnitude;
            const angleDiff = (targetAngle - currentAngle + 180) % 360 - 180;
            if (Math.abs(angleDiff) > 1.5) {
                this.seekTarget = undefined;
                this.seekTurning = undefined;
            }
            else {
                this.facing += Math.sign(angleDiff) * Math.min(this.seekTurning, Math.abs(angleDiff));
                this.moveAngle = this.facing;
                this.velocity.x = Math.cos(this.facing) * speed;
                this.velocity.y = Math.sin(this.facing) * speed;
            }
        }
    }
...
export class Mob extends Entity {
    ...
    destroy() {
        ...
        // ADDITION
        const clients = state.clients.values().toArray();
        const dropConfig = mobConfigs[this.index].drops;
        const mobDrops = [];
        for (let i = 0; i < dropConfig.length; i++) {
            const drop = dropConfig[i];
            if (Math.random() > drop.chance) continue;
            const rarity = getDropRarity(this.rarity, Infinity);
            if (rarity < drop.minRarity) continue;
            mobDrops.push({index: drop.index, rarity: rarity, x: this.x, y: this.y});
        }
        for (let i = 0; i < mobDrops.length; i++) {
            const drop = mobDrops[i];
            drop.x += Math.cos(i / mobDrops.length * Math.PI * 2) * 30;
            drop.y += Math.sin(i / mobDrops.length * Math.PI * 2) * 30;

            const linked = [];
            for (let j = 0; j < clients.length; j++) {
                linked.push(new Drop(drop, clients[j], drop.index, drop.rarity))
            }
        ...
        topDamagers.forEach(damager => {
            const client = state.clients.get(damager.clientID);
            if (client) client.addXP((Math.random() * 0.3 + 0.7) * Math.pow(3, this.rarity + 1));
        });
    ...
    update() {
        ...
        if (distSqr < Math.pow(this.size + this.client.body.size + this.client.body.extraPickupRange, 2)) {
            if (this.client.pickupDrop(this)) {
                ...
                // ADDITION
                if (this.linkedDrops) {
                    if (this.linkCount <= 1) this.linkedDrops?.forEach(drop => {drop?.destroy()});
                    else {
                        const linkCount = this.linkCount - 1;
                        this.linkedDrops?.forEach(drop => {drop.linkCount = linkCount});
                    }
                }                
            }
        }
